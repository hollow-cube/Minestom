...
public class AnvilLoaderIntegrationTest {
    ...
    public void loadHouse(Env env) {
        ...
        AnvilLoader chunkLoader = new AnvilLoader(env.minecraftServer(), worldFolder) {
    ...
    public void loadAndSaveChunk(Env env) throws InterruptedException {
        Instance instance = env.createFlatInstance(new AnvilLoader(env.minecraftServer(), worldFolder) {
            // Force loads inside current thread
            @Override
            public boolean supportsParallelLoading() {
                return false;
