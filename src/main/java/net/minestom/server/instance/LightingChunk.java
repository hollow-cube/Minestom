...
public class LightingChunk extends DynamicChunk {
    ...
    public LightingChunk(@NotNull Instance instance, int chunkX, int chunkZ) {
        ...
        this.lightCache = new CachedPacket(instance::getServerSettings, this::createLightPacket);
    }
    ...
    public void sendLighting() {
        if (!isLoaded()) return;
        sendPacketToViewers(instance::getServerSettings, lightCache);
    }
    ...
    static void updateAfterGeneration(LightingChunk chunk) {
        ...
        sendingTask = chunk.instance.getSchedulerManager().scheduleTask(() -> {
            queueLock.lock();
