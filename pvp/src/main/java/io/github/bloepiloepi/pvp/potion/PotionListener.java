...
public class PotionListener {
    ...
    public static EventNode<EntityInstanceEvent> events(PotionConfig config) {
        ...
        node.addListener(PlayerEatEvent.class, event -> {
            if (event.getItemStack().material() == Material.POTION) {
        ...
        if (config.isSplashEnabled())
            node.addListener(PlayerUseItemEvent.class, event -> {
                if (event.getItemStack().material() == Material.SPLASH_POTION){
        ...
        if (config.isLingeringEnabled())
            node.addListener(PlayerUseItemEvent.class, event -> {
                if (event.getItemStack().material() == Material.LINGERING_POTION) {
                    ThreadLocalRandom random = ThreadLocalRandom.current();
